...
for(i in 1:length(prop))
{
  ...
  st2[i]=system.time(makeConsensus2(cl, proportion=prop[i], algorithm="cluster_walktrap"))[[3]]
...
for(i in 1:length(prop))
{
  ...
  st3[i]=system.time(makeConsensus2(cl, proportion=prop[i], algorithm="cluster_louvain"))[[3]]
...
for(i in 1:length(prop))
{
  mc4=makeConsensus2(cl, proportion=prop[i], algorithm="components_weak")
  nclust4[i]=length(table(mc4$clustering))
  st4[i]=system.time(makeConsensus2(cl, proportion=prop[i], algorithm="components_weak"))[[3]]
...
for(i in 1:length(prop))
{
  mc5=makeConsensus2(cl, proportion=prop[i], algorithm="components_strong")
  nclust5[i]=length(table(mc5$clustering))
  st5[i]=system.time(makeConsensus2(cl, proportion=prop[i], algorithm="components_strong"))[[3]]
...
plot(nclust2,col=2,type="l",xlab="proportion",xaxt="n",main="makeConsensus2")
axis(1, at=1:length(prop), labels=prop)
points(nclust3,col=3,type="l")
legend("topright",legend=c("walktrap","louvain"),col=c("red","green"),lty=1,cex=0.8)
plot(nclust4,col=4,type="l",xlab="proportion",xaxt="n",main="makeConsensus2 - components")
axis(1, at=1:length(prop), labels=prop)

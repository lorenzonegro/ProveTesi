...
sim=analisi(ngruppi=10,n_cells=1000,range=c(-155.5,155.5))
nclust_mc=sim[[1]][,1]
nclust_lo=sim[[2]][,1]
...
numero_cellule=c(3000,5000,7000,10000)

#prima simulazione casuale
set.seed(123)
for(i in 1:length(numero_cellule))
{
  cat(i)
  sim=analisi(ngruppi=10,n_cells=numero_cellule[i],range=c(-155.5,155.5))
...
save(output,file="Statistiche cambio numero cellule.RData")

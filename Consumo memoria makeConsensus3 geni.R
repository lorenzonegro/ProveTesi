...
Rprof(filename ="ram_consensus3.txt", append = FALSE, memory.profiling = TRUE)
mc3=makeConsensus3(cl, proportion=0.5, algorithm="cluster_louvain")
Rprof(NULL)

profile2 <- summaryRprof(filename = "ram_consensus3.txt", chunksize = -1L, 
                         memory = "tseries", diff = FALSE)
max_mem_cons3 <- max(rowSums(profile2[,1:3]))*0.00000095367432
consumo_memoria=max_mem_cons3
...
profile2 <- summaryRprof(filename = "ram_consensus3.txt", chunksize = -1L, 
                         memory = "tseries", diff = FALSE)
max_mem_cons3 <- max(rowSums(profile2[,1:3]))*0.00000095367432
consumo_memoria=cbind(consumo_memoria,max_mem_cons3)
...
output[["Memoria mC3(Louvain)"]] <- consumo_memoria
